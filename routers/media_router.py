...
async def run_clean_mix(
    ...
    # Call service method without user_id (anonymous)
    return await MixService.run_clean_mix(request, None)
...
@media_router.post("/mix/{session_id}")
async def mix_audio(session_id: str):
    """Basic mix endpoint using apply_basic_mix"""
    return await MixService.mix_audio(None, session_id)


@media_router.post("/mix/process-single")
async def process_single_mix(
    ...
    Args:
        ...
        session_id: Optional session ID (if not provided, generated)
    ...
    # Ensure input directory exists (anonymous, no user_id)
    input_dir = Path(f"./media/{session_id}/mix")
    ...
    try:
        ...
        # Generate public URL (anonymous, no user_id)
        mix_url = f"/media/{session_id}/mix/master.wav"
